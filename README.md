# Hi there! I'm Ananyot Damnoenkiat 👋
### 🚀 Aspiring Data Engineer | Data Science & Business Analytics Graduate

I am an ambitious and fast-learning individual passionate about data and technology. My focus is on building robust data solutions, designing scalable ETL/ELT pipelines, and leveraging Cloud Computing (GCP) to ensure data availability and integrity.

---

### 🛠️ Technical Skills

| Category | Tools & Technologies |
| :--- | :--- |
| **Data Engineering** | ![Data Pipeline](https://img.shields.io/badge/-Data-Pipeline-8ECAE6) ![Data Modeling](https://img.shields.io/badge/-Data-Modeling-219EBC) ![Data Warehousing](https://img.shields.io/badge/-Data-Warehousing-023047)
| **Programming** | ![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white) ![SQL](https://img.shields.io/badge/-SQL-4479A1?logo=postgresql&logoColor=white) |
| **Data Processing** | ![Apache Spark](https://img.shields.io/badge/-Apache%20Spark-E25A1C?logo=apache-spark&logoColor=white) ![Pandas](https://img.shields.io/badge/-Pandas-150458?logo=pandas&logoColor=white) |
| **Orchestration** | ![Apache Airflow](https://img.shields.io/badge/-Apache%20Airflow-017CEE?logo=apache-airflow&logoColor=white) |
| **Cloud Platform** | ![GCS](https://img.shields.io/badge/-GCS-4285F4?logo=google-cloud&logoColor=white) ![BigQuery](https://img.shields.io/badge/-BigQuery-4285F4?logo=google-cloud&logoColor=white) |
| **DevOps & Tools** | ![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=white) ![Terraform](https://img.shields.io/badge/-Terraform-7B42BC?logo=terraform&logoColor=white) ![Git](https://img.shields.io/badge/-Git-F05032?logo=git&logoColor=white) ![dbt](https://img.shields.io/badge/-dbt-fb8500?logo=dbt&logoColor=white)|

---

### 🌟 Featured Projects

#### 🛒 [Retail Project - Big Data ELT & Quality Pipeline](https://github.com/ananyot-damnoenkiat/retail_project)
A scalable ELT pipeline utilizing **PySpark** for data transformation and **Airflow** for orchestration. Features automated data quality checks and a Star Schema modeling approach in BigQuery.
* **Key Tech:** PySpark, Airflow, Docker, GCP, Terraform

#### 📈 [WallStreet Data Ops - Automated Financial Data Pipeline](https://github.com/ananyot-damnoenkiat/wallstreet-data-ops)
An automated pipeline extracting financial data via APIs with a focus on **idempotency** and error handling. Managed via Terraform on Google Cloud Platform.
* **Key Tech:** Python, Airflow, GCP, Terraform, dbt

---

### 🎓 Education & Certifications
* **B.Sc. in Data Science and Business Analytics** | King Mongkut's Institute of Technology Ladkrabang
* **Road to Data Engineer 3.0** | Data th School
    * Focus: Spark Cleansing, GCP Cloud Computing, and Airflow Pipelines

---

### 📫 Contact Me:
* **Email:** ananyot.damnoenkiat@gmail.com
* **GitHub:** [github.com/ananyot-damnoenkiat](https://github.com/ananyot-damnoenkiat)
